...
import { ArrowDownCircle, ArrowUpCircle, AlertTriangle, Trophy, ShieldX, Clock, Gift, ArrowLeft } from 'lucide-react'; // Import ArrowLeft
...
const History = () => {
  ...
  useEffect(() => {
    const fetchHistory = async () => {
      ...
      try {
        ...
        // 4. Fetch all other transactions
        ...
        const harufBets = harufBetsSnapshot.docs.map(doc => {
          ...
          let statusMessage = `Haruf Bet Placed on ${data.selectedNumber} in ${data.marketName}`;

          if (data.status === 'win') {
            type = 'win';
            displayAmount = data.winnings || data.betAmount; // Use winnings if available
            statusMessage = `Haruf Win on ${data.selectedNumber} in ${data.marketName}`;
          } else if (data.status === 'loss') {
            type = 'loss';
            statusMessage = `Haruf Loss on ${data.selectedNumber} in ${data.marketName}`;
          }
        ...
        const combinedHistory = [...deposits, ...withdrawals, ...bets, ...referralBonuses, ...harufBets, ...rouletteBets, ...adminCredits];
  ...
  return (
    <div className="font-roboto bg-gray-900 text-white min-h-screen p-4"> {/* Removed pt-20 here */}
      <div
        className="absolute top-4 left-4 p-2 cursor-pointer bg-gray-700 rounded-full text-white"
        onClick={() => navigate(-1)}
      >
        <ArrowLeft size={24} />
      </div>
      <div className="max-w-2xl mx-auto pt-16"> {/* Added pt-16 here to push content down */}
